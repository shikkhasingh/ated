  ...
  "AuthConnector" must {
    ...
    "agentReferenceNo" must {
      "Check that this returns the agent reference number if we have one" in {
        ...
        when(mockWSHttp.GET[HttpResponse](Matchers.any())(Matchers.any(), Matchers.any())).thenReturn(Future.successful(HttpResponse(OK, Some(successResponseJson))))
      ...
      "returns None if the status from Auth isn't OK" in {
        ...
        when(mockWSHttp.GET[HttpResponse](Matchers.any())(Matchers.any(), Matchers.any())).thenReturn(Future.successful(HttpResponse(BAD_REQUEST, Some(successResponseJson))))
